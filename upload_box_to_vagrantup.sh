...
upload_boxfile_to_vagrantup() {
  ...
  echo "*** Getting current version of the box from Vagrant Cloud"
  ...
  if ( [ "$CURRENT_VERSION" == "$BOX_VERSION" ] && ! curl -s "https://app.vagrantup.com/api/v1/box/$VAGRANT_CLOUD_USER/$NAME" | jq .versions[].providers[].name | grep -q "$VAGRANT_PROVIDER" ) || [ "$CURRENT_VERSION" != "$BOX_VERSION" ] ; then
    ...
    echo  "*** The box with version \"$BOX_VERSION\" for \"$VAGRANT_PROVIDER\" provider already exists in Vagrant Cloud. Skipping upload..."
  fi
}
