...
(
  ...
  for PACKER_VAGRANT_PROVIDER in libvirt virtualbox; do
    for BUILD in ubuntu-{20,18,16}.04-desktop-amd64 ubuntu-{20,18,16}.04-server-amd64 my_ubuntu-{20,18,16}.04-server-amd64 my_centos-7-x86_64 {my_,}windows-1{0,1}-enterprise-x64-eval windows-server-{2022,2019,2016,2012_r2}-standard-x64-eval; do
      echo "**** $(date)"
